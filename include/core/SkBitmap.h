...
public:
    ...
    */
    ...
    /*AndroidCompile: Creates a color palette for bitmaps with less than 256 colors (used in doMyEncode) 
	       Fills the newImage with 1 color index per byte
	table    - A pointer to a 256 color table
	count    - Holds the number of colors found 
        newImage - Points to a structure that should hold one byte per color index

	If there are more than 255 colors then function returns false
 */
    bool createColorPalette (SkPMColor* table, uint8_t& count, char* newImage) const;

    /**AndroidCompile: Returns true if the color was found in the palette. Currently this is a very naive implementation */
    int findIndexInPalatte(SkPMColor* table, uint8_t count, SkPMColor color) const;
