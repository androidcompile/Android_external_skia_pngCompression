...
/**AndroidCompile: Finds the index of a certain color in the palette */
int SkBitmap::findIndexInPalatte(SkPMColor* table, uint8_t count, SkPMColor color) const {
...
/**AndroidCompile: Currently supporting only bitmaps whose size doesn't exceed 64 bits
	    Also, this function ignores alpha transparency 
	    If there are more than 255 colors then function returns false */
bool SkBitmap::createColorPalette (SkPMColor table[], uint8_t& count, char* newImage) const {
    const char* srcImage = (const char*)fPixels;
    const SkPMColor* SK_RESTRICT srcP = (const SkPMColor*)srcImage;
    int size = fWidth*fHeight;

    count = 0;
    for (int i = 0; i < size; ++i) {
	SkPMColor c = *srcP++; 
	//c = c & alphaMask;//0x00FFFFFF;
	int colorIndex = findIndexInPalatte(table, count, c);
	if (colorIndex == -1) {
	    //Add color to palette and increase count (naive implementation - need to sort colors)	
	    table[count] = c;// & 0x00FFFFFF;
	    colorIndex = count;
	    if (count == 255)
		return false;
	    count++;
	}
	newImage[i] = (char) colorIndex; 	
    } 	
    return true;
}
///////////////////////////////////////////////////////////////////////////////
